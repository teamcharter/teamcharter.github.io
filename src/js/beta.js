...
function main(user) {
	...
	initWithTeamCode((tid) => {
		...
		let startTab = params.tab || 'charter';
		onTabClick(tabList, document.querySelectorAll(`.charter-tab[data-tab="container-${startTab}"]`)[0]);
...
function mainProgressTab(user, tid) {
	...
	database.onTeamChange(tid, (team, members) => {
		...
		if (!isTeamTemplate) {
			renderProgressTab(tid, team, members);
		}
...
function renderProgressTab(tid, team, members) {
